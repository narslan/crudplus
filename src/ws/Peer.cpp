...
public:
    ...
    Action act() override
    {
      ...
      return oatpp::async::synchronize(
                 ...
                 std::move(m_websocket->sendOneFrameTextAsync(m_message).next(
...
Peer::handleFileChunkMessage(const oatpp::Object<MessageDto>& message)
{
  ...
  if(!file)
    return nullptr; // Ignore if file doesn't exist. File may be deleted already.
...
oatpp::async::CoroutineStarter Peer::onPing(const std::shared_ptr<AsyncWebSocket>& socket,
                                            const oatpp::String& message)
{
  return oatpp::async::synchronize(&m_writeLock, socket->sendPongAsync(message));
}

oatpp::async::CoroutineStarter Peer::onPong(const std::shared_ptr<AsyncWebSocket>& socket,
                                            const oatpp::String& message)
{
  --m_pingPoingCounter;
...
oatpp::async::CoroutineStarter Peer::onClose(const std::shared_ptr<AsyncWebSocket>& socket,
                                             v_uint16 code,
                                             const oatpp::String& message)
{
  return nullptr; // do nothing
